...
document.querySelectorAll(".help").forEach( help => help.addEventListener("click", doHelpPopUp.bind(help)) );
function doHelpPopUp() {
	//console.log("clicked");
	var info = this.querySelector("span.info"), d = this.dataset, leftOffset, infoWidth, windowWidth, leftPosition, helpHTML, tooltipID;
	if(this.classList.contains("popOut")) {
		...
		if(tooltipID !== undefined) {
			...
			let s = info.style, newTop = this.offsetTop + this.scrollHeight;
			s.position = "absolute";
			s.top = newTop.toString() + "px";
			s.left = leftOffset.toString() + "px";
			s.zIndex = zCounter.toString();
			info.addEventListener("click", function(e) {
				var me = e.currentTarget, z = document.querySelector("span[data-clicked=\"" + me.getAttribute("id") + "\"]");
