...
function oneSyllable(a,b,c,e){b=syllPatternPick(b,e);for(e=0;e<b.length;e++){var d=b.charAt(e);if(-1===cat.index.indexOf(d))a+=d;else{d=cat[d];var h=0===c?Math.random()*d.length:powerLaw(d.length,c);d=d.charAt(h);a+=d}}return a}function syllPatternPick(a,b){switch(a){case -1:var c=nwisyl;var e=wisyl;break;case 0:c=nsyl;e=syl;break;case 1:c=nwfsyl;e=wfsyl;break;case 2:c=nsnsyl,e=snsyl}return e[powerLaw(c,calcDropoff(c,b))]}
function getOneWord(a,b,c,e,d,h){var f=1,k,g="";Math.random()>b&&(f+=1+powerLaw(4,50));if(1===f)g=oneSyllable("",c?-1:2,d,h);else for(b=1;b<=f;b++)c||1===b?k=-1:1<b&&(k=b<f?0:1),g=oneSyllable(g,k,d,h),e&&b<f&&(g+="\u00b7");g=applyRewriteRules(g);a&&(g=g.charAt(0).toUpperCase()+g.substring(1));return g}
function createText(a,b,c,e,d){var h,f,k="",g=getAdvancedNumber("#sentences",30,500);for(h=0;h<g;h++){var l=1+peakedPowerLaw(15,5,50);for(f=0;f<l;f++)k+=getOneWord(0===f,a,b,c,e,d),f===l-1&&(k+=".....?!".charAt(Math.floor(7*Math.random()))),k+=" "}return k}
function createLex(a,b,c,e,d,h){var f,k='<div class="lexicon" style="grid-template-columns: repeat(auto-fit, minmax('+getAdvancedNumber("#wordLengthInEms",10,1E3)+'em, 1fr) )">\n',g=getAdvancedNumber("#lexiconLength",150,1E3);for(f=0;f<g;f++)k+="<div>"+getOneWord(a,b,c,e,d,h)+"</div>";return k+"</table></div>\n"}function createLongLex(a,b,c,e,d){var h,f="",k=getAdvancedNumber("largeLexiconLength",750,5E3);for(h=0;h<5*k;h++)f+=getOneWord(!1,a,b,c,e,d)+"<br>\n";return f}
...
function recurseCategories(a,b,c){var e=[];e.push.apply(e,$jscomp.arrayFromIterable(c));c=e.shift();-1===cat.index.indexOf(c)?(b+=c,e.length?recurseCategories(a,b,e):a.add(applyRewriteRules(b))):0<e.length?cat[c].split("").forEach(function(c){recurseCategories(a,b+c,e)}):cat[c].split("").forEach(function(c){a.add(applyRewriteRules(b+c))})}function escapeHTML(a){a=document.createTextNode(a);var b=document.createElement("p");b.appendChild(a);return b.innerHTML}
function process(){var a,b=[];var c=$("input[type=radio][name=outtype]:checked").val();var e=$("#showsyl").prop("checked");var d=$("#slowsyl").prop("checked");var h=$("#onetype").prop("checked");var f=Number($("input[type=radio][name=monosyl]:checked").val());var k=Number($("input[type=radio][name=dropoff]:checked").val());f!==f||1<f?f=1:0>f&&(f=0);k!==k||45<k?k=45:0>k&&(k=0);var g=$("#cats").val();if(g!==previousCat){var l=g.split(/\r?\n/);ncat=l.length;cat={index:""};(l=l.every(function(c){c=c.trim();
var d=c.length;if(0===d)ncat--;else{if(3>d||1!==c.indexOf("=")||-1!==c.indexOf("=",2))return b.push("<strong>Error:</strong>"+escapeHTML(c)+"<br>Categories must be of the form V=aeiou<br>That is, a single letter, an equal sign, then a list of possible expansions."),!1;a=c.charAt(0);if(cat.hasOwnProperty(a))return b.push("<strong>Error:</strong> You have defined category "+escapeHTML(a)+" more than once."),!1;cat.index+=c.charAt(0);cat[a]=c.substring(2)}return!0}))&&0<ncat&&(previousCat=g)}g=$("#syls").val();
g!==previousSyl&&(syl=parseSyllables(g),nsyl=syl.length,previousSyl=g);g=$("#wrdi").val();g!==previousWisyl&&(wisyl=parseSyllables(g),nwisyl=wisyl.length,previousWisyl=g);g=$("#wrdf").val();g!==previousWfsyl&&(wfsyl=parseSyllables(g),nwfsyl=wfsyl.length,previousWfsyl=g);g=$("#sing").val();g!==previousSnsyl&&(snsyl=parseSyllables(g),nsnsyl=snsyl.length,previousSnsyl=g);g=$("#rewrite").val();if(previousRew!==g){l=g.split(/\r?\n/);nrew=l.length;rew={};badrew=!1;var n=0;(l=l.every(function(a){var c=a.indexOf("||");
if(""===a.trim())nrew--;else{if(1>c||c!==a.lastIndexOf("||"))return b.push("<strong>Error:</strong>"+escapeHTML(a)+"<br>Rewrite rules must be in the form x||y<br>That is, a rule (x), followed by two vertical bars, followed by a replacement expression (y, which may be blank)."),!1;var d=a.substring(c+2);a=handleCategoriesInRewriteRule(a.substring(0,c));a=new RegExp(a,"g");rew[n.toString()]=[a,d];n++}return!0}))&&(previousRew=g)}0>=ncat&&b.push("<strong>Missing:</strong> You must have categories to generate text.");
h&&0>=nwisyl?b.push("<strong>Missing:</strong> You must have syllable types to generate text."):!h&&(0>=nsyl||0>=nwisyl||0>=nwfsyl||0>=nsnsyl)&&b.push("<strong>Missing:</strong> You must have <em>all</em> syllable types to generate text.");if(0<b.length)var m=b.join("<br><br>");else switch(c){case "text":m=createText(f,h,e,k,d);break;case "dict":m=createLex(!1,f,h,e,k,d);break;case "dictC":m=createLex(!0,f,h,e,k,d);break;case "longdict":m=createLongLex(f,h,e,k,d);break;case "genall":m=getEverySyllable()}$("#outputText").html(m)}
function calcDropoff(a,b){if(1===a)return 101;if(b)switch(a){case 2:return 50;case 3:return 40;case 4:case 5:case 6:case 7:case 8:case 9:return 46-4*a;default:return 9}return 9>a?60-5*a:12}function erase(){$("#outputText").html("")}function clearBoxes(){$("#cats,#wrdi,#syls,#sing,#wrdf,#rewrite").val("");$("#monoLessFrequent,#dropoffMedium,#textOutput").prop("checked",!0);$("#showsyl,#slowsyl").prop("checked",!1);$("#defaultName").text("")}
function prepImport(){$("#importBoxArea").show();$("body").css("overflow","hidden")}function removeImportBox(){$("#importTextBox").val("");$("#importBoxArea").hide();$("body").css("overflow","auto")}
...
$("#"+a[9]).prop("checked",!0);$("#"+a[10]).prop("checked",!0);syllablicChangeDetection($("#onetype")[0]);alert("Import Successful!");removeImportBox()}
function doExport(){var a="--CATS--\n"+$("#cats").val()+"\n--REWRITE--\n"+$("#rewrite").val()+"\n--MONO--\n"+$("#sing").val()+"\n--MID--\n"+$("#syls").val()+"\n--INIT--\n"+$("#wrdi").val()+"\n--FINAL--\n"+$("#wrdf").val()+"\n--FLAGS--\n"+($("#onetype").prop("checked")?"1":"0")+" "+($("#slowsyl").prop("checked")?"1":"0")+" "+$("input[name=monosyl]:checked").attr("id")+" "+$("input[name=dropoff]:checked").attr("id");$("#importTextBox").val(a);prepImport();alert("Export Successful. Copy this for your own records.\n\nHit 'Cancel' when you're done.")}
...
"undefined"!==typeof Storage&&(Customizable=!0,null!==localStorage.getItem("CustomCats")&&(CustomInfo=!0,$("#predef").prepend('<option value="-1">Custom</option>'),$("#predef").val("-1"),loadThisPredef(-1)));$("#onetype").change(function(){syllablicChangeDetection($("#onetype")[0])});syllablicChangeDetection($("#onetype")[0]);zCounter=1;
$(".help").click(function(){var a=$(this).find("span.info");if($(this).hasClass("popOut")){var b=$(this).attr("data-clicked");if("undefined"!==typeof b)$("#"+b).remove(),$(this).removeAttr("data-clicked");else{var c=$(this).offset();var e=a.html();++zCounter;b="popup"+zCounter;$(this).attr("data-clicked",b);$("body").append('<div class="info" id="'+b+'"></div>');a=$("#"+b);a.toggle();b=c.left;var d=a.width()+20;var h=$(document).width();var f=b+$(this).width();f+d>h&&(b=f<=d?h<=d?b-f+2:b+(h-(f+d))-
3:b-d);a.html(e).css({"z-index":zCounter,top:c.top+10,left:b});a.click(function(){$('span[data-clicked="'+$(this).attr("id")+'"]').removeAttr("data-clicked");$(this).remove()})}}else a.toggle(),a.css("transform","translateX(0px)"),a.is(":visible")&&(++zCounter,a.css("z-index",zCounter),d=a.width()+20,h=$(document).width(),c=$(this).offset(),f=c.left+$(this).width(),f+d>h&&(f<=d?h<=d?a.css("transform","translateX("+(2-f)+"px)"):a.css("transform","translateX("+(h-f-d)+"px)"):a.css("transform","translateX("+
(0-d)+"px)")))});
