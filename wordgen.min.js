...
$("#largeLexiconLength").val()+"\n"+$("#sentences").val()+"\n"+$("#rewSep").val();$("#importTextBox").val(a);prepImport();doAlert("","Copy this for your own records.<br><br>Hit 'Cancel' when you're done.","info")}
function saveCustom(a){a=void 0===a?!1:a;if(Customizable){if(CustomInfo&&!a)return doConfirm("Warning!","You already have information saved. Do you want to overwrite it?",function(){saveCustom(!0)},function(){doAlert("Previous information saved.","Nothing overwritten.","success")},"warning","Yes","No");localStorage.setItem("CustomCats",$("#cats").val());localStorage.setItem("CustomRewrite",$("#rewrite").val());localStorage.setItem("CustomSing",$("#sing").val());localStorage.setItem("CustomSyls",$("#syls").val());
localStorage.setItem("CustomWrdi",$("#wrdi").val());localStorage.setItem("CustomWrdf",$("#wrdf").val());localStorage.setItem("CustomOneType",$("#onetype").prop("checked"));localStorage.setItem("CustomSlowsyl",$("#slowsyl").prop("checked"));localStorage.setItem("CustomMono",$("input[name=monosyl]:checked").attr("id"));localStorage.setItem("CustomDropoff",$("input[name=dropoff]:checked").attr("id"));localStorage.setItem("CustomWordLengthInEms",$("#wordLengthInEms").val());localStorage.setItem("CustomLexiconLength",
$("#lexiconLength").val());localStorage.setItem("CustomLargeLexiconLength",$("#largeLexiconLength").val());localStorage.setItem("CustomSentences",$("#sentences").val());localStorage.setItem("CustomRewSep",$("#rewSep").val());CustomInfo=!0;1>$("#predef option[value=-1]").length&&$("#predef").prepend('<option value="-1">Custom</option>');$("#predef").val("-1");doAlert("Saved to browser.","","success")}else doAlert("","Your browser does not support Local Storage and cannot save your information.","error")}
function clearCustom(a){a=void 0===a?!1:a;if(Customizable)if(CustomInfo){if(CustomInfo&&!a)return doConfirm("Warning!","Are you sure you want to delete your saved settings?",function(){clearCustom(!0)},function(){doAlert("Settings kept.","","success")},"warning","Yes","No");"CustomCats CustomRewrite CustomSing CustomSyls CustomWrdi CustomWrdf CustomOneType CustomSlowsyl CustomMono CustomDropoff CustomWordLengthInEms CustomLexiconLength CustomLargeLexiconLength CustomSentences CustomRewSep".split(" ").forEach(function(a){window.localStorage.removeItem(a)});
CustomInfo=!1;$("#predef option[value=-1]").remove();doAlert("Cleared from browser.","","success")}else doAlert("","You don't have anything saved.","error");else doAlert("Sorry!","Your browser does not support Local Storage and cannot save your information.","error")}function showipa(){$("#outputText").html(returnIPAPlus('<span class="desc">','</span>\n<div class="extraGroup">',"</div>\n","<br><br>"))}function advancedOptions(){$("#advancedOpen").toggleClass("closed")}
...
function doAlert(a,b,c){void 0!==Swal?Swal.fire({type:c,title:a,html:b,customClass:"alertBox",buttonsStyling:!1}):alert(a+" "+b.replace(/<br>/g,"\n").replace(/<[^>]*>/g,""))}
function doConfirm(a,b,c,d,f,h,g){void 0!==Swal?Swal.fire({title:a,html:b,type:void 0===f?"question":f,showCancelButton:!0,confirmButtonText:void 0===h?"Ok":h,cancelButtonText:void 0===g?"Cancel":g,customClass:"alertBox",buttonsStyling:!1}).then(function(a){return a.value?c():d()}):confirm(a+" "+b.replace(/<br>/g,"\n").replace(/<[^>]*>/g,""))?c():d()};
