...
function handleCategoriesInRewriteRule(a){a=a.split("%%");for(var b=[],c,d,e,f;a.length;){c=a.shift().split("!%");for(e=c.shift();c.length;)f=c.shift(),d=f.charAt(0),-1!==cat.index.indexOf(d)&&(e+="[^"+cat[d]+"]"),e+=f.substring(1);c=e.split("%");for(e=c.shift();c.length;)f=c.shift(),d=f.charAt(0),-1!==cat.index.indexOf(d)&&(e+="["+cat[d]+"]"),e+=f.substring(1);b.push(e)}return b.join("%")}
...
function oneSyllable(a,b,c,d){b=syllPatternPick(b,d);for(d=0;d<b.length;d++){var e=b.charAt(d);if(-1===cat.index.indexOf(e))a+=e;else{e=cat[e];var f=0===c?Math.random()*e.length:powerLaw(e.length,c);e=e.charAt(f);a+=e}}return a}function syllPatternPick(a,b){switch(a){case -1:var c=nwisyl;var d=wisyl;break;case 0:c=nsyl;d=syl;break;case 1:c=nwfsyl;d=wfsyl;break;case 2:c=nsnsyl,d=snsyl}return d[powerLaw(c,calcDropoff(c,b))]}
function getOneWord(a,b,c,d,e,f){var g=1,k,h="";Math.random()>b&&(g+=1+powerLaw(4,50));if(1===g)h=oneSyllable("",c?-1:2,e,f);else for(b=1;b<=g;b++)c||1===b?k=-1:1<b&&(k=b<g?0:1),h=oneSyllable(h,k,e,f),d&&b<g&&(h+="\u00b7");h=applyRewriteRules(h);a&&(h=h.charAt(0).toUpperCase()+h.substring(1));return h}
function createText(a,b,c,d,e){var f,g,k="",h=getAdvancedNumber("#sentences",30,500);for(f=0;f<h;f++){var l=1+peakedPowerLaw(15,5,50);for(g=0;g<l;g++)k+=getOneWord(0===g,a,b,c,d,e),g===l-1&&(k+=".....?!".charAt(Math.floor(7*Math.random()))),k+=" "}return k}
function createLex(a,b,c,d,e,f){var g,k='<div class="lexicon" style="grid-template-columns: repeat(auto-fit, minmax('+getAdvancedNumber("#wordLengthInEms",10,1E3)+'em, 1fr) )">\n',h=getAdvancedNumber("#lexiconLength",150,1E3);for(g=0;g<h;g++)k+="<div>"+getOneWord(a,b,c,d,e,f)+"</div>";return k+"</table></div>\n"}function createLongLex(a,b,c,d,e){var f,g="",k=getAdvancedNumber("largeLexiconLength",750,5E3);for(f=0;f<5*k;f++)g+=getOneWord(!1,a,b,c,d,e)+"<br>\n";return g}
...
function process(){var a,b=[];var c=$("input[type=radio][name=outtype]:checked").val();var d=$("#showsyl").prop("checked");var e=$("#slowsyl").prop("checked");var f=$("#onetype").prop("checked");var g=Number($("input[type=radio][name=monosyl]:checked").val());var k=Number($("input[type=radio][name=dropoff]:checked").val());g!==g||1<g?g=1:0>g&&(g=0);k!==k||45<k?k=45:0>k&&(k=0);var h=$("#cats").val();if(h!==previousCat){var l=h.split(/\r?\n/);ncat=l.length;cat={index:""};(l=l.every(function(c){c=c.trim();
var d=c.length;if(0===d)ncat--;else{if(3>d||1!==c.indexOf("=")||-1!==c.indexOf("=",2))return b.push("<strong>Error:</strong>"+escapeHTML(c)+"<br>Categories must be of the form V=aeiou<br>That is, a single letter, an equal sign, then a list of possible expansions."),!1;a=c.charAt(0);if(cat.hasOwnProperty(a))return b.push("<strong>Error:</strong> You have defined category "+escapeHTML(a)+" more than once."),!1;cat.index+=c.charAt(0);cat[a]=c.substring(2)}return!0}))&&0<ncat&&(previousCat=h)}h=$("#syls").val();
h!==previousSyl&&(syl=parseSyllables(h),nsyl=syl.length,previousSyl=h);h=$("#wrdi").val();h!==previousWisyl&&(wisyl=parseSyllables(h),nwisyl=wisyl.length,previousWisyl=h);h=$("#wrdf").val();h!==previousWfsyl&&(wfsyl=parseSyllables(h),nwfsyl=wfsyl.length,previousWfsyl=h);h=$("#sing").val();h!==previousSnsyl&&(snsyl=parseSyllables(h),nsnsyl=snsyl.length,previousSnsyl=h);h=$("#rewrite").val();if(previousRew!==h){l=h.split(/\r?\n/);nrew=l.length;rew={};badrew=!1;var n=0;(l=l.every(function(a){var c=a.indexOf("||");
if(""===a.trim())nrew--;else{if(1>c||c!==a.lastIndexOf("||"))return b.push("<strong>Error:</strong>"+escapeHTML(a)+"<br>Rewrite rules must be in the form x||y<br>That is, a rule (x), followed by two vertical bars, followed by a replacement expression (y, which may be blank)."),!1;var d=a.substring(c+2);a=handleCategoriesInRewriteRule(a.substring(0,c));a=new RegExp(a,"g");rew[n.toString()]=[a,d];n++}return!0}))&&(previousRew=h)}0>=ncat&&b.push("<strong>Missing:</strong> You must have categories to generate text.");
f&&0>=nwisyl?b.push("<strong>Missing:</strong> You must have syllable types to generate text."):!f&&(0>=nsyl||0>=nwisyl||0>=nwfsyl||0>=nsnsyl)&&b.push("<strong>Missing:</strong> You must have <em>all</em> syllable types to generate text.");if(0<b.length)var m=b.join("<br><br>");else switch(c){case "text":m=createText(g,f,d,k,e);break;case "dict":m=createLex(!1,g,f,d,k,e);break;case "dictC":m=createLex(!0,g,f,d,k,e);break;case "longdict":m=createLongLex(g,f,d,k,e);break;case "genall":m=getEverySyllable()}$("#outputText").html(m)}
...
function showipa(){var a='<span class="desc">Latin:</span>\n<div class="extraGroup">',b=[161,191,216,248,11360,42786,42891,42928,43824,43872,"x",592,"x",880,886,890,902,904,908,910,931,43877,"x",1024,1162,42560,42624,"x",1329,1377],c=[161,214,246,591,11391,42887,42925,42935,43866,43876,"IPA",767,"Greek and Coptic",883,887,895,902,906,908,929,1023,43877,"Cyrillic",1154,1327,42606,42651,"Armenian",1366,1415],d,e;var f=0;for(e=b.length;f<e;f++)if("x"===b[f])a+='</div>\n<br><br><span class="desc">'+c[f]+
':</span>\n<div class="extraGroup">';else for(d=b[f];d<=c[f];d++)a+='<span title="'+d+": "+getUnicodeName(d)+'">'+String.fromCharCode(d)+"</span>";a+="</div>\n";$("#outputText").html(a)}function advancedOptions(){$("#advancedOpen").toggleClass("closed")}
...
$(".help").click(function(){var a=$(this).find("span.info");if($(this).hasClass("popOut")){var b=$(this).attr("data-clicked");if("undefined"!==typeof b)$("#"+b).remove(),$(this).removeAttr("data-clicked");else{var c=$(this).offset();var d=a.html();++zCounter;b="popup"+zCounter;$(this).attr("data-clicked",b);$("body").append('<div class="info" id="'+b+'"></div>');a=$("#"+b);a.toggle();b=c.left;var e=a.width()+20;var f=$(document).width();var g=b+$(this).width();g+e>f&&(b=g<=e?f<=e?b-g+2:b+(f-(g+e))-
3:b-e);a.html(d).css({"z-index":zCounter,top:c.top+10,left:b});a.click(function(){$('span[data-clicked="'+$(this).attr("id")+'"]').removeAttr("data-clicked");$(this).remove()})}}else a.toggle(),a.css("transform","translateX(0px)"),a.is(":visible")&&(++zCounter,a.css("z-index",zCounter),e=a.width()+20,f=$(document).width(),c=$(this).offset(),g=c.left+$(this).width(),g+e>f&&(g<=e?f<=e?a.css("transform","translateX("+(2-g)+"px)"):a.css("transform","translateX("+(f-g-e)+"px)"):a.css("transform","translateX("+
(0-e)+"px)")))});
